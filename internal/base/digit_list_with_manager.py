...
class DigitListWithManager(DigitList):
    ...
    def remove_checked_lines(self) -> int:
        # filter digits to remove
        list_to_remove = []
        for i in range(0, len(self._list), self._columns):
            checked_row = [elem for elem in self._list[i:i + self._columns] if elem.is_checked()]
    ...
    def check_digit_buttons(self, first_x: int, first_y: int, second_x: int, second_y: int) -> int:
        # check values
        first_digit_value = self.get_digit_by_x_y(first_x, first_y).get_value()
        second_digit_value = self.get_digit_by_x_y(second_x, second_y).get_value()
        ...
        for check_func in [
            ...
            must_check, score = check_func(first_x, first_y, second_x, second_y)
            if must_check:
                # set check for digits
                self.check_digit_by_x_y(first_x, first_y)
                self.check_digit_by_x_y(second_x, second_y)
                return score
        return 0

    # one row, one column or one diagonal neighbours
    @staticmethod
    def _is_neighbours(first_x: int, first_y: int, second_x: int, second_y: int) -> (bool, int):
    ...
    def _is_one_row(self, first_x: int, first_y: int, second_x: int, second_y: int) -> (bool, int):
        ...
        for x in range(min(first_x, second_x) + 1, max(first_x, second_x)):
            if not self.get_digit_by_x_y(x, first_y).is_checked():
                return False, 0
        return True, abs(first_x - second_x)

    # digits placed on one column
    def _is_one_column(self, first_x: int, first_y: int, second_x: int, second_y: int) -> (bool, int):
        # check digits placed on one column
        if first_x != second_x:
            return False, 0
        # check all digits between two given digits is checked
        for y in range(min(first_y, second_y) + 1, max(first_y, second_y)):
            if not self.get_digit_by_x_y(first_x, y).is_checked():
                return False, 0
        return True, abs(first_y - second_y)

    # digits placed on diagonal
    def _is_diagonal(self, first_x: int, first_y: int, second_x: int, second_y: int) -> (bool, int):
        ...
        for i in range(min_x + 1, max_x):
            if not self.get_digit_by_x_y(i, i).is_checked():
                return False, 0
        return True, max_x - min_x

    # digits are neighbours in list representation (not matrix)
    def _is_line_end(self, first_x: int, first_y: int, second_x: int, second_y: int) -> (bool, int):
        first_digit = self.get_digit_by_x_y(first_x, first_y)
        second_digit = self.get_digit_by_x_y(second_x, second_y)

        first_digit_idx = self._get_digit_index(first_digit)
        second_digit_idx = self._get_digit_index(second_digit)

        for i in range(min(first_digit_idx, second_digit_idx) + 1, max(first_digit_idx, second_digit_idx)):
            if not self._list[i].is_checked():
                return False, 0
